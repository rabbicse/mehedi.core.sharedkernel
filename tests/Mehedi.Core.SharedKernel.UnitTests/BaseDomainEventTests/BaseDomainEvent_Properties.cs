...
{
    private sealed record OrderPlacedEvent(Guid OrderId) : BaseDomainEvent(nameof(OrderPlacedEvent), OrderId);
    ...
    public void StoresAggregateId()
    {
        ...
        evt.AggregateId.Should().Be(orderId);
    }
    ...
    public void SameParametersHaveSameMessageTypeAndAggregateId()
    {
        ...
        // OccurredOn is set to DateTime.UtcNow at construction time so two separate
        // instances differ by timestamp — test individual properties instead.
    ...
    private sealed record BaseDomainEvent_NullMessageType(Guid Id) : BaseDomainEvent(null, Id);
}
